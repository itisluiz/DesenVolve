...
import { Container, Paper, Stack, Typography, Dialog, DialogContent, DialogTitle, Fab, Icon, TextField, DialogActions, Button, Drawer, List, ListItem, Box, ClickAwayListener } from '@mui/material';
import { useContext, useEffect, useState } from 'react';
import { apiProjetos, apiObterEquipe, apiAdicionarMembro, apiRemoverMembro, apiRemoverEquipe } from '../api/equipe';
...
const PaginaProjetos = (props) => {
	...
	const [carregandoAdicionarMembro, setCarregandoAdicionarMembro] = useState(false);
	...
	function cancelarCriarProjeto() { setCriarProjetoAberto(false); setNomeNovoProjeto(''); }
	function cancelarAdicionarMembro() { setAdicionarMembroAberto(false); setEmailNovoMembro(''); setBloquarClickaway(false); }
	function cancelarSairEquipe() { setSairEquipeAberto(false); }
	...
	async function atualizarDadosEquipe()
	{
		apiObterEquipe(codigo).then(equipe => {
			...
			setEquipe(equipe);
			setCargoUsuario(equipe.membrosEquipe.find(membro => membro.usuario.codigo == usuario.codigo).cargo);
		});
	}
	...
	useEffect(() => { atualizarProjetosEquipe(); atualizarDadosEquipe(); }, [codigo]);

	return (
		<Container sx={{ display: 'flex', mt: '1.5em' }}>
			<Drawer variant="temporary" open={drawerMembrosAberta} anchor="left" position="fixed" sx={{ zIndex: 99 }} >
				<ClickAwayListener onClickAway={() => !bloquearClickaway && setDrawerMembrosAberta(false)}>
				<Box sx={{ width: '20em' }}>
					<List sx={{mt: '3.5em'}}>
						<ListItem>
							<div>
								<Typography variant="h6" color="primary" mb="-0.5em">Membros</Typography>
								<Typography variant="caption">Lista de membros da equipe</Typography>
							</div>

							{cargoUsuario && cargoUsuario.codigo >= 1 &&
								<Fab onClick={() => {setAdicionarMembroAberto(true); setBloquarClickaway(true);}} sx={{ml: 'auto', width: '3em', height: '3em'}} variant="contained" color="primary"><Icon>add</Icon></Fab>}
						</ListItem>
						
						{equipe && equipe.membrosEquipe.map((membro) => (
							<ListItem key={membro.usuario.nome}>
								<CardMembro fnOnPopup={setBloquarClickaway} fnAttMembros={atualizarDadosEquipe} codigoEquipe={equipe.codigo} membro={membro} cargoUsuario={cargoUsuario} />
							</ListItem>
						))}
					</List>
				</Box>
			...
			<Button onClick={() => setDrawerMembrosAberta(true)} variant="contained" color="primary" sx={{ position: 'fixed', top: '6em', left: '1em', zIndex: 98 }}><Icon sx={{mr: '0.3em'}}>group</Icon>Membros</Button>
			...
			<Dialog
				open={criarProjetoAberto}
				onClose={cancelarCriarProjeto}
			>
				<DialogTitle>Criar novo projeto</DialogTitle>
				<DialogContent>
					<TextField value={nomeNovoProjeto} onChange={(e) => setNomeNovoProjeto(e.target.value)} autoFocus placeholder="Nome do projeto" label="Nome" fullWidth variant="standard" />
				</DialogContent>
				<DialogActions>
					<Button onClick={cancelarCriarProjeto}>Cancelar</Button>
					<LoadingButton onClick={submitCriarProjeto} loading={carregandoCriarProjeto} disabled={nomeNovoProjeto.length < 2}>Criar</LoadingButton>
				</DialogActions>
			</Dialog>
			<Dialog
				open={adicionarMembroAberto}
				onClose={cancelarAdicionarMembro}
			>
				<DialogTitle>Adicionar membro na equipe</DialogTitle>
				<DialogContent>
					<TextField value={emailNovoMembro} onChange={(e) => setEmailNovoMembro(e.target.value)} autoFocus placeholder="E-mail do integrante" label="E-mail" fullWidth variant="standard" />
				</DialogContent>
				<DialogActions>
					<Button onClick={cancelarAdicionarMembro}>Cancelar</Button>
					<LoadingButton onClick={submitAdicionarMembro} loading={carregandoAdicionarMembro} disabled={emailNovoMembro.length < 2}>Adicionar</LoadingButton>
				</DialogActions>
			</Dialog>

			<Dialog
				open={sairEquipeAberto}
				onClose={cancelarSairEquipe}
			>
				<DialogTitle color='error'>Sair da equipe</DialogTitle>
				<DialogContent>
					<Typography variant="caption" component="p">Você tem certeza que deseja sair da equipe <b>{equipe?.nome}</b>?</Typography>
					{cargoUsuario?.codigo == 2 &&
						<Typography color="error" variant="h6" component="p">Você é líder da equipe, ao sair, a equipe será desfeita.</Typography>
					}

				</DialogContent>
				<DialogActions>
					<Button onClick={cancelarSairEquipe}>Cancelar</Button>
					<Button color="error" onClick={submitSairEquipe}>Sair</Button>
				</DialogActions>
			</Dialog>
		</Container>
	);
