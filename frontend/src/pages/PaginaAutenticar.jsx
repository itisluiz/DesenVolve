...
const PaginaAutenticar = (props) => {
  ...
  useEffect(() => {
    apiLogado().then((usr) => contextualizarUsuario(usr));
  }, []);
  ...
  function submitCadastro(nome, sobrenome, email, senha) {
    setCarregando(true);
    apiCadastro(nome, sobrenome, email, senha).then((usr) =>
      contextualizarUsuario(usr)
    );
  }
  ...
  return (
    <Container sx={{ display: "flex", justifyContent: "center" }}>
      <Paper sx={{ width: "60%", mt: "3em" }}>
        ...
        {aba === 1 && (
          <FormCadastro loading={carregando} submit={submitCadastro} />
        )}
      </Paper>
    </Container>
  );
