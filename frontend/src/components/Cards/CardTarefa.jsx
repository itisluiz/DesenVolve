...
const CardTarefa = (props) => {
  ...
  if (props.tarefa.complexidade == 1) corComplexidade = "#008F39";
  else if (props.tarefa.complexidade == 2) corComplexidade = "#FFA500";
  else if (props.tarefa.complexidade == 3) corComplexidade = "#FF0000";
  else corComplexidade = "#AAAAAA";

  function temPermissaoSobre() {
    if (props.tarefa.responsavel.codigo == usuario.codigo) return true;
  ...
  function obterBotaoAcao() {
    if (temPermissaoSobre()) {
      if (!props.tarefa.inicio)
        return (
          <LoadingButton
            loading={carregandoEstado}
            onClick={submitIniciarTarefa}
            color="success"
            sx={{ ml: "auto" }}
            variant="contained"
          >
    ...
    if (props.tarefa.inicio && props.tarefa.finalizado)
      return (
        <Icon color="success" sx={{ ml: "auto" }}>
          done
        </Icon>
      );
    else if (props.tarefa.inicio)
      return (
        <Icon color="warning" sx={{ ml: "auto" }}>
          timer
        </Icon>
      );
    ...
    <Card>
      <CardContent sx={{ display: "flex" }}>
        <div style={{ width: "100%" }}>
          ...
          <span style={{ display: "flex" }}>
            <Typography color="primary" mr="0.3em" fontWeight="bold">
              Responsável:
            </Typography>
            <Avatar
              sx={{ width: "1em", height: "1em", mr: "0.3em" }}
              src={props.tarefa.responsavel.urlAvatar}
            />
            <Typography component="h2">
              {props.tarefa.responsavel.nomeDisplay}
            </Typography>
          </span>

          <span style={{ display: "flex" }}>
            <Typography mr="0.3em" fontWeight="bold">
              Complexidade:
            </Typography>
            <Typography
              sx={{
                mt: "0.15em",
                height: "1.5em",
                backgroundColor: corComplexidade,
                p: "0em 0.4em 0em 0.4em",
                borderRadius: "0.5em",
                opacity: 0.7,
                fontSize: "0.8em",
              }}
          ...
          <div style={{ width: "100%" }}>
            <Typography>
              <Icon
                color="success"
                sx={{ mr: "0.35em", mb: "-0.1em", fontSize: "0.9em" }}
              >
                calendar_today
              </Icon>
              Início:{" "}
              <Typography variant="caption">
                {!props.tarefa.inicio
                  ? "Não iniciado"
                  : formatptbr(props.tarefa.inicio)}
              </Typography>
            </Typography>
            <Typography>
              <Icon
              ...
              Término:{" "}
              <Typography variant="caption">
                {!props.tarefa.finalizado
                  ? "Não finalizado"
                  : formatptbr(props.tarefa.finalizado)}
              </Typography>
            </Typography>
            <Typography>
              <Icon
